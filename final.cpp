...
bool block_1(int arr[][COLS], int rows)
{
    bool gameover=false;
    for (int i = 0; i < 12; i++)
    {
    ...
    for (int j = start + 1; j <= length_horizontal + start && !gameover; j++)
    {
        if(arr[0][j]==1)
        {
            gameover=true;
...
bool block_2(int arr[][COLS], int rows)
{
    bool gameover=false;
    for (int i = 0; i < 12; i++)
    {
...
bool block_3(int arr[][COLS], int rows)
{
    bool gameover=false;
    for (int i = 0; i < 12; i++)
    {
...
bool block_4(int arr[][COLS], int rows)
{
    bool gameover=false;
    for (int i = 0; i < 12; i++)
    {
...
bool block_5(int arr[][COLS], int rows)
{
    bool gameover;
    for (int i = 0; i < 12; i++)
    {
    ...
    if(arr[0][start]==1 || arr[0][start+1]==1|| arr[1][start+1]==1 || arr[1][start+2]==1)
    {
        gameover=true;
        return gameover;
    }
...
bool block_6(int arr[][COLS], int rows)
{
    bool gameover=false;
    for (int i = 0; i < 12; i++)
    {
    ...
    int length = 2;
    int start = rand() % (COLS - 2 - length) + 0;
    if(arr[0][start+1]==1||arr[1][start]==1||arr[1][start+1]==1|| arr[1][start+2]==1)
    {
        gameover=true;
        return gameover;
    }
...
bool block_7(int arr[][COLS], int rows)
{
    bool gameover=false;
    for (int i = 0; i < 12; i++)
    {
    ...
    int length = 2;
    int start = rand() % (COLS - 2 - length) + 0;
    if(arr[0][start]==1||arr[1][start]==1||arr[1][start + 1]==1||arr[2][start])
    {
        gameover=true;
        return gameover;
    }
...
bool spawnNewPiece(int arr[][COLS])
{
    bool gameover;
    int newblock = rand() % 7 + 1;
    if (newblock == 1)
    {
        gameover=block_1(arr, ROWS);
    }
    else if (newblock == 2)
    {
        gameover=block_2(arr, ROWS);
    }
    else if (newblock == 3)
    {
        gameover=block_3(arr, ROWS);
    }
    else if (newblock == 4)
    {
        gameover=block_4(arr, ROWS);
    }
    else if (newblock == 5)
    {
        gameover=block_5(arr, ROWS);
    }
    else if (newblock == 6)
    {
        gameover=block_6(arr, ROWS);
    }
    else
    {
        gameover=block_7(arr, ROWS);
    }
    return gameover;
}
int main()
{
    ...
    while(playAgain)
    {
        ...
        while (window.isOpen() && gameOverFlag == false)
        {
            ...
            if (!activeBlock)
            {
                gameOverFlag=spawnNewPiece(arr);
                if(!gameOverFlag)
                {
                    activeBlock = true;
                }
            ...
            if (gravityElapsed.count() >= gravitySpeed&&!gameOverFlag) // if (time of gravity passes ) only then move the blocks  down
            {
                lastGravity = now;
